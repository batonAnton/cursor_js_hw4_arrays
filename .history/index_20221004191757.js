...
for ( i = 0; i < boys.length; i++) {
    pairs[i] =     ' ' + "Студенти: " + boys[i] + "," + girls[i];
  }
...
let subjects = [];
for(i = 0; i < boys.length; i++){
    subjects.push( ''  + pairs[i] + "," + " " +"предмет -  " +themes[i]);
}
...
for(i = 0;i < marks.length; i++){
    rating.push('' + "Студент: " + "" + "" +students[i] + ";" +" " + "Оцінка -  " + marks[i]);
}
...
for(i = 0; i < subjects.length; i++){
    randomMarks.push(subjects[i] + ";" +" " + "Оцінка - " + getRandomMarks());
}
